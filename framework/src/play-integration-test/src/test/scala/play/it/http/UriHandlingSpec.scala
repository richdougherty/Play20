...
class UriHandlingSpec extends PlaySpecification with AllEndpointsIntegrationSpecification with EndpointOkHttpClientSupport with ApplicationFactories {

  private def makeRequest[T: AsResult](uri: String)(block: (ServerEndpoint, okhttp3.Response) => T): Fragment = withRouter { components: BuiltInComponents =>
    ...
    val response: okhttp3.Response = okUrl(uri)
    block(endpointClient.endpoint, response)
  }
