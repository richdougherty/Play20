...
object AssetsSpec extends PlaySpecification {
  "Assets controller" should {
    ...
    "serve an asset with the right mime type, despite a percent-encoded suffix" in withServer {
      val result = await(wsUrl("/bar.t%78t").get())
    ...
    "forbid access to directories" in withServer {
      val result = await(wsUrl("/").get())

      result.status must_== FORBIDDEN
      result.body must beEmpty
    }
    ...
    "forbid access to attempts to escape the assets directory" in withServer {
      val result = await(wsUrl("/../testassets/bar.txt").get())

      result.status must_== FORBIDDEN
      result.body must beEmpty
    }

    "forbid access to filenames with percent-encoded slashes" in withServer {
      val result = await(wsUrl("/subdir%2fbaz.txt").get())
