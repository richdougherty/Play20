...
object Concurrent {
  ...
  def broadcast[E]: (Enumerator[E], Channel[E]) = {
    ...
    val toPush = new Channel[E] {
      ...
      private val sequentialRunner = new SequentialRunner()
      def sequential(body: => Unit) = sequentialRunner.runSynchronous(body)(dec)

      def push(chunk: Input[E]) = sequential {
      ...
      def end(e: Throwable) = sequential {
  ...
  def unicast[E](
    ...
    def apply[A](it: Iteratee[E, A]): Future[Iteratee[E, A]] = {
      ...
      val pushee = new Channel[E] {
        ...
        def push(item: Input[E]) = sequential {
