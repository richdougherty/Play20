...
/**
 * Sequentially executes asynchronous and synchronous operations. Operations
 * are added to a schedule, then each item in the schedule is executed in
 * order.
 *
 ...
 * operation. The SequentialRunner runner performs both scheduling and
 * executions of operations internally without the need for a separate
 * thread. This means the SequentialRunner doesn't consume any resources
 * when it isn't being used.
 *
 * No locks are held by this class, only atomic operations are used.
 */
private[play] final class SequentialRunner {

  import SequentialRunner._

  /**
   * The state of the SequentialRunner, either Inactive or Runnning.
   */
   ...
   */
  def run[A](body: => Future[A])(implicit ec: ExecutionContext): Unit = {
    schedule(Op(() => body.asInstanceOf[Future[Unit]], ec.prepare))
  }

  /**
   * Schedule an operation to be run. The operation is considered
   * complete when it finishes executing. In other words, will begin
   ...
   * The operation will execute in the given ExecutionContext.
   */
  def runSynchronous(body: => Unit)(implicit ec: ExecutionContext): Unit = {
    run {
      body
      Future.successful(())
    }
   ...
   * are pending, then the SequentialRunner will enter an inactive state.
...
private object SequentialRunner {
  ...
  sealed trait State
  /**
   * The state of a SequentialRunner that is neither running an operation nor
   * has any pending operations.
   */
  final case object Inactive extends State
  /**
   * The state of a SequentialRunner that is currently running a task and has
